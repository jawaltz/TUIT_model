...
Subj_Sess = unique(trialData(:,[tdef.subj]), 'rows'); % number of subjects and sessions
...
for s = 1:size(Subj_Sess,1)   % for fitting across group of subs, loop through each one, resetting V, Go/NoGo etc.
    ...
    avgRT = mean(trialData(trialData(:,tdef.subj) == this_subj,tdef.rt));
...
%         emo = toFit(1, tdef.emo); %emotion
        ...
        % [RTpred, ret] = TC_Alg(fitRT, reward, params, priors, avgRT, rewFunc, model);
