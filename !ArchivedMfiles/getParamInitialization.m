function [init_params, lower_limits, upper_limits] = getParamInitialization(model,f)
...
if strcmp(model, 'orig') || strcmp(model, 'orig_scram')
    ...
    %   ,7:  exp_alt          #alternative exponential models for RT swings
    %   (not sure of its use yet)---removed
    ...
    upper_limits = [1 ; 10000; 1 ; 1 ; 5000 ; 5000 ; 10000 ]; % for rmsearch set min/max to 0 for unused params (otherwise spits out weird values that aren't used)
    hdr = {'Subject','Session','lambda','explore','alphaG','alphaN','K','nu','rho','SSE'};
elseif strcmp(model, 'origsticky') || strcmp(model, 'origsticky_scram')
    ...
    %    ,8:  meandiff         #rho parameter: weight for expected reward of fast versus slow
    init_params = [ 0.3 ; 2000 ; 0.2 ; 0.2 ; 1000 ; 0.1 ; 300 ];
    lower_limits = [ 0 ; -10000 ; 0.01 ; 0.01 ; .1 ; 0 ; 0 ];
    upper_limits = [1 ; 10000 ; 1 ; 1 ; 5000 ; 1 ; 10000 ]; % for rmsearch set min/max to 0 for unused params (otherwise spits out weird values that aren't used)
    hdr = {'Subject','Session','lambda','explore','alphaG','alphaN','K','sticky_decay','rho','SSE'};
elseif strcmp(model, 'origstickyNew') 
    %NOEMOSTICKY params 8 x 1 <numeric>
    %    ,1:  lambda           #weight for sticky choice (weight influencing prior RTs' effect on
    %                              current RT. This is a decaying function of RT history; see sticky_decay)
    %    ,2:  explore          #epsilon parameter: how much should RT be modulated by greater relative uncertainty
    ...
    lower_limits = [ 0 ; 1 ; 0.01 ; 0.01 ; .1 ; 0 ; .1 ; 0 ];
    upper_limits = [1 ; 100000 ; 1 ; 1 ; 5000 ; 1 ; 5000 ; 10000 ]; % for rmsearch set min/max to 0 for unused params (otherwise spits out weird values that aren't used)
