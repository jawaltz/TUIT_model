function [RTpred, ret]=TC_Alg(RTobs, Reward, params, priors, avg_RT, rewFunc, model)
...
% if nargin < 8, model = 'orig'; end %if not specified, use the model where no parameters vary by emotion.
...
if strcmp(model, 'intercept') %|| strcmp(model, 'orig_scram')
    ...
    %    ,5:  K                #baseline response speed (individual mean RT?)
    ...
    lambda = params(1);
    explore = params(1); % all parameters except K will be meaningless
    alpha1 =  params(3);
    alpha2 = params(4);
...
elseif strcmp(model, 'orig') %|| strcmp(model, 'orig_scram')
    %NOEMO params 8 x 1 <numeric>
    %    ,1:  lambda           #weight for previous trial RT (autocorrelation of RT_t with RT_t-1)
    %    ,2:  explore          #epsilon parameter: how much should RT be modulated by greater relative uncertainty
    %                              about fast vs. slow responses
...
elseif strcmp(model, 'origstickyNew') %|| strcmp(model, 'origsticky_scram')
    %NOEMOSTICKY params 8 x 1 <numeric>
    %    ,1:  lambda           #weight for sticky choice (weight influencing prior RTs' effect on
    %                              current RT. This is a decaying function of RT history; see sticky_decay)
    %    ,2:  explore          #epsilon parameter: how much should RT be modulated by greater relative uncertainty
...
ret.along =  NaN(numTrials, 1); ret.ashort =  NaN(numTrials, 1);
ret.blong =  NaN(numTrials, 1); ret.bshort =  NaN(numTrials, 1);
...
%     end
    ...
    ret.along(trial-1) = alph_long; ret.ashort(trial-1) = alph_short;
    ret.blong(trial-1) = b_long; ret.bshort(trial-1) = b_short;
    ...
    if (trial == numTrials)
        ...
        ret.along(trial) = alph_long; ret.ashort(trial) = alph_short;
        ret.blong(trial) = b_long; ret.bshort(trial) = b_short;
    end
