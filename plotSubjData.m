...
for b = 1:length(blocks)
    ...
    set(gca, 'Box', 'off' );
    saveas(fig, ['../outputs/figures/S' num2str(subjid) '/S' num2str(subjid) '_' model '_' rewFuncNames{ret_all(b).block}  '_block' num2str(blocks(b)) '.jpg']);
end
...
%     hs=subplot(2,2,b);
%     
%     plot(1:ntrials, smooth(ret_all(Rew_block==b).rtpred,1),'LineWidth', 2);
%     hold on
%     
%     axis([0 ntrials 0 trialLength]);
...
% print(fig, ['../outputs/figures/S' num2str(subjid) '/S' num2str(subjid) '_' model '_' 'Reward'], '-djpeg100  ', '-r300')
% close(fig);
...
for b = 1:length(blocks)
    ...
    title(['Subject: ' num2str(subjid) ', ' rewFuncNames{ret_all(b).rewFunc}],  'FontSize', 32);
    legend('Exploration Param','RT Swing')
    ylabel('RT (ms)',  'FontSize', 24);
    xlabel('Trial', 'FontSize', 24);
    set(gca, 'Box', 'off' );
    saveas(fig, ['../outputs/figures/S' num2str(subjid) '/S' num2str(subjid) '_' model '_' rewFuncNames{ret_all(b).block}  '_' '_RTSwing.jpg']);
end
